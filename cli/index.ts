...
const checkConfig = () => {
  ...
  msg += 'Please launch Hdtyper and retry.';
  console.error(msg);
  process.exit(1);
};
...
const lsRemote = (pattern?: string) => {
  ...
  const URL = `https://api.npms.io/v2/search?q=${
    (pattern && `${pattern}+`) || ''
  }keywords:hdtyper-plugin,hdtyper-theme&size=250`;
  return got(URL)
    ...
    commandPromise = lsRemote(query)
      .then((entries) => {
        if (entries.length === 0) {
          ...
          console.error(`${chalk.red('Try')} ${chalk.green('hdtyper ls-remote')}`);
          process.exit(1);
        } else {
...
args.command('<default>', 'Launch Hdtyper');
...
const main = (argv: string[]) => {
  ...
  const env = Object.assign({}, process.env, {
    // this will signal Hdtyper that it was spawned from this module
    HDTYPER_CLI: '1',
  ...
  if (!flags.verbose) {
    ...
    if (process.platform === 'darwin') {
      //Use `open` to prevent multiple Hdtyper process
      const cmd = `open -b co.zeit.hdtyper ${args_}`;
      const opts = {
        env
      };
