...
const applicationDirectory =
  process.env.XDG_CONFIG_HOME !== undefined
    ? path.join(process.env.XDG_CONFIG_HOME, 'hdtyper')
    : process.platform == 'win32'
    ? path.join(process.env.APPDATA!, 'Hdtyper')
    : os.homedir();

const devConfigFileName = path.join(__dirname, `../.hdtyper.js`);

const fileName =
  process.env.NODE_ENV !== 'production' && fs.existsSync(devConfigFileName)
    ? devConfigFileName
    : path.join(applicationDirectory, '.hdtyper.js');

/**
 * We need to make sure the file reading and parsing is lazy so that failure to
 * statically analyze the hdtyper configuration isn't fatal for all kinds of
