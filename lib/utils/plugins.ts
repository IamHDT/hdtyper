...
// https://github.com/vercel/hdtyper/issues/619
...
let connectors: {
  ...
  Hdtyper: {state: any[]; dispatch: any[]};
...
Module._load = function _load(path: string) {
  ...
  switch (path) {
    ...
    case 'react-dom':
      ...
      return ReactDOM;
    case 'hdtyper/component':
      console.warn(
        'DEPRECATED: If your plugin requires `hdtyper/component`, it must requires `react.PureComponent` instead and bundle `react` as a dependency'
      );
      return PureComponent;
    case 'hdtyper/notify':
      return notify;
    case 'hdtyper/Notification':
      return Notification;
    case 'hdtyper/decorate':
      return decorate;
    default:
...
const loadModules = () => {
  ...
  modules = paths.plugins
    ...
    .map((path: any) => {
      ...
      // mapHdtyperTermState mapping for backwards compatibility with hdtyperterm
      if (mod.mapHdtyperTermState) {
        mod.mapHdtyperState = mod.mapHdtyperTermState;
        console.error('mapHdtyperTermState is deprecated. Use mapHdtyperState instead.');
      }

      // mapHdtyperTermDispatch mapping for backwards compatibility with hdtyperterm
      if (mod.mapHdtyperTermDispatch) {
        mod.mapHdtyperDispatch = mod.mapHdtyperTermDispatch;
        console.error('mapHdtyperTermDispatch is deprecated. Use mapHdtyperDispatch instead.');
      }
      ...
      if (mod.mapHdtyperState) {
        connectors.Hdtyper.state.push(mod.mapHdtyperState);
      }

      if (mod.mapHdtyperDispatch) {
        connectors.Hdtyper.dispatch.push(mod.mapHdtyperDispatch);
      }
...
export function connect<stateProps, dispatchProps>(
  stateFn: (state: HdtyperState) => stateProps,
  dispatchFn: (dispatch: HdtyperDispatch) => dispatchProps,
  c: any,
  d: Options = {}
) {
  return (Class: any, name: keyof typeof connectors) => {
    return reduxConnect<stateProps, dispatchProps, any, HdtyperState>(
      (state) => {
