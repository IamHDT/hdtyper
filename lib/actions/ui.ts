...
import {HdtyperState, HdtyperDispatch, HdtyperActions} from '../hdtyper';
import {Stats} from 'fs';

const {stat} = window.require('fs');

export function openContextMenu(uid: string, selection: any) {
  return (dispatch: HdtyperDispatch, getState: () => HdtyperState) => {
    dispatch({
...
export function resetFontSize(): HdtyperActions {
  return {
...
export function windowGeometryUpdated(): HdtyperActions {
  return {
...
const findChildSessions = (termGroups: HdtyperState['termGroups']['termGroups'], uid: string): string[] => {
...
const getGroupUids = (state: HdtyperState) => {
...
export function enterFullScreen(): HdtyperActions {
  return {
    type: UI_ENTER_FULLSCREEN
  };
}

export function leaveFullScreen(): HdtyperActions {
  return {
...
export function execCommand(command: string, fn: (e: any, dispatch: HdtyperDispatch) => void, e: any) {
  return (dispatch: HdtyperDispatch) =>
    dispatch({
