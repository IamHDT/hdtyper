...
import {HdtyperState, session, HdtyperDispatch, HdtyperActions} from '../hdtyper';
...
function createExitAction(type: typeof SESSION_USER_EXIT | typeof SESSION_PTY_EXIT) {
  return (uid: string) => (dispatch: HdtyperDispatch, getState: () => HdtyperState) => {
    return dispatch({
      type,
      uid,
      effect() {
...
export function clearActiveSession(): HdtyperActions {
  return {
    type: SESSION_CLEAR_ACTIVE
  };
}

export function setSessionXtermTitle(uid: string, title: string): HdtyperActions {
  return {
