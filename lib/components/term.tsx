...
export default class Term extends React.PureComponent<TermProps> {
    ...
    if (!props.term) {
      ...
      if (props.webGLRenderer) {
        if (needTransparency) {
          ...
          // Experimental WebGL renderer needs some more glue-code to make it work on Hdtyper.
  ...
  getTermDocument() {
    console.warn(
      'The underlying terminal engine of Hdtyper no longer ' +
