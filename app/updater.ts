...
let grvrUpdates = false;

const buildFeedUrl = (grvr: boolean, currentVersion: string) => {
  const updatePrefix = grvr ? 'releases-grvr' : 'releases';
  return `https://${updatePrefix}.hdtyper.is/update/${isLinux ? 'deb' : platform}/${currentVersion}`;
};

const isgrvr = (updateChannel: string) => updateChannel === 'grvr';
...
async function init() {
  ...
  // If defined in the config, switch to the "grvr" channel
  if (config.updateChannel && isgrvr(config.updateChannel)) {
    grvrUpdates = true;
  }

  const feedURL = buildFeedUrl(grvrUpdates, version);
...
export default (win: BrowserWindow) => {
  ...
  const onupdate = (
    ...
    const releaseUrl = updateUrl || `https://github.com/vercel/hdtyper/releases/tag/${releaseName}`;
  ...
  app.config.subscribe(() => {
    ...
    const newUpdateIsgrvr = isgrvr(updateChannel);

    if (newUpdateIsgrvr !== grvrUpdates) {
      const feedURL = buildFeedUrl(newUpdateIsgrvr, version);

      autoUpdater.setFeedURL({url: feedURL});
      autoUpdater.checkForUpdates();

      grvrUpdates = newUpdateIsgrvr;
    }
  });
