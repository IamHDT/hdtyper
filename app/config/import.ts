...
// Migrate Hdtyper2 config to Hdtyper3 but only if the user hasn't manually
// touched the new config and if the old config is not a symlink
const migrateHdtyper2Config = () => {
  if (cfgPath === legacyCfgPath) {
    // No need to migrate
    return;
  }
  if (!existsSync(legacyCfgPath)) {
    // Already migrated or user never used Hdtyper 2
    return;
  }
  ...
  if (existsNew) {
    ...
    if (hasNewConfigBeenTouched) {
      ...
      notify(
        'Hdtyper 3',
        `Settings location has changed to ${cfgPath}.\nWe've backed up your old Hdtyper config to ${backupPath}`
      );
      return;
    }
  }
