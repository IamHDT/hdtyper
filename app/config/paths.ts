...
const applicationDirectory =
  process.env.XDG_CONFIG_HOME !== undefined
    ? join(process.env.XDG_CONFIG_HOME, 'hdtyper')
...
const legacyCfgPath = join(homeDirectory, cfgFile); // Hdtyper 2 config location
...
const plugins = resolve(cfgDir, '.hdtyper_plugins');
const plugs = {
  legacyBase: resolve(homeDirectory, '.hdtyper_plugins'),
  legacyLocal: resolve(homeDirectory, '.hdtyper_plugins', 'local'),
  base: plugins,
  local: resolve(plugins, 'local'),
  cache: resolve(plugins, 'cache')
};
const yarn = resolve(__dirname, '../../bin/yarn-standalone.js');
const cliScriptPath = resolve(__dirname, '../../bin/hdtyper');
const cliLinkPath = '/usr/local/bin/hdtyper';
