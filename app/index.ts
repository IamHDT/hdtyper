// Print diagnostic information for a few arguments instead of running Hdtyper.
if (['--help', '-v', '--version'].includes(process.argv[1])) {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  const {version} = require('./package');
  const configLocation = process.platform === 'win32' ? `${process.env.userprofile}\\.hdtyper.js` : '~/.hdtyper.js';
  console.log(`Hdtyper version ${version}`);
  console.log('Hdtyper does not accept any command line arguments. Please modify the config file instead.');
  console.log(`Hdtyper configuration file located at: ${configLocation}`);
  process.exit();
}
    ...
    .then(() => {
      ...
      if (!isDev) {
        ...
        if (config.getConfig().defaultSSHApp && !app.isDefaultProtocolClient('ssh')) {
          console.log('Setting Hdtyper as default client for ssh:// protocol');
          app.setAsDefaultProtocolClient('ssh');
        } else if (!config.getConfig().defaultSSHApp && app.isDefaultProtocolClient('ssh')) {
          console.log('Removing Hdtyper from default client for ssh:// protocol');
