...
function updatePlugins({force = false} = {}) {
  ...
  install((err: any) => {
    ...
    if (err) {
      ...
      const changed = cache.get('hdtyper.plugin-versions') !== pluginVersions && loaded === total;
      cache.set('hdtyper.plugin-versions', pluginVersions);
...
// to prevent slowness
if (cache.get('hdtyper.plugins') !== id || process.env.HDTYPER_FORCE_UPDATE) {
...
function syncPackageJSON() {
  ...
  const pkg = {
    name: 'hdtyper-plugins',
    description: 'Auto-generated from `~/.hdtyper.js`!',
    private: true,
    version: '0.0.1',
    repository: 'vercel/hdtyper',
    license: 'MIT',
    homepage: 'https://hdtyper.is',
    dependencies
  };
...
function requirePlugins(): any[] {
  ...
  const load = (path_: string) => {
    ...
    try {
      ...
      if (!exposed) {
        notify('Plugin error!', `${`Plugin "${basename(path_)}" does not expose any `}Hdtyper extension API methods`);
        return;
      }
