...
const addSymlink = async (silent: boolean) => {
  try {
    ...
    if (isInstalled) {
      console.log('Hdtyper CLI already in PATH');
      return;
    }
    console.log('Linking HdtyperCLI');
    ...
    if (err.code === 'EACCES' && !silent) {
      const result = await dialog.showMessageBox({
        message: `You need to grant elevated privileges to add Hdtyper CLI to PATH
Or you can run
sudo ln -sf "${cliScriptPath}" "${cliLinkPath}"`,
      ...
      if (result.response === 0) {
        try {
          await sudoExec(`ln -sf "${cliScriptPath}" "${cliLinkPath}"`, {name: 'Hdtyper'});
          return;
        } catch (_error) {
          error = _error[0];
        }
...
const addBinToUserPath = () => {
  return new Promise<void>((resolve, reject) => {
    ...
    try {
      ...
      // C:\Users\<user>\AppData\Local\Programs\hdtyper\resources\bin
      const binPath = path.dirname(cliScriptPath);
      // C:\Users\<user>\AppData\Local\hdtyper
      const oldPath = path.resolve(process.env.LOCALAPPDATA!, 'hdtyper');
      ...
      console.log('Adding HdtyperCLI path (registry)');
...
export const installCLI = async (withNotification: boolean) => {
  if (process.platform === 'win32') {
    try {
      ...
      logNotify(
        withNotification,
        'Hdtyper CLI installed',
        'You may need to restart your computer to complete this installation process.'
      );
    } catch (err) {
      logNotify(withNotification, 'Hdtyper CLI installation failed', `Failed to add Hdtyper CLI path to user PATH ${err}`);
    }
    ...
    try {
      ...
      logNotify(withNotification, 'Hdtyper CLI installed', `Symlink created at ${cliLinkPath}`);
    } catch (error) {
      logNotify(withNotification, 'Hdtyper CLI installation failed', `${error}`);
    }
  } else {
    logNotify(withNotification, 'Hdtyper CLI installation failed', `Unsupported platform ${process.platform}`);
  }
};
